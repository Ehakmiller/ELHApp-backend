...
# DEPLOY / DEV SWITCHES
# ==========================
DEPLOY_TO_GITHUB = False   # <-- set True when you want to push to GitHub (TEMPORARILY FALSE TO DEBUG)
...
exec(open(r"C:\Users\ehakm\OneDrive\Documents\Python Code\Ethanol db\Basis_dataframe\Basis_df15.py",
...
def _is_real_number(x) -> bool:
    # True for ints/floats (anheader_html = viewport_meta + cssd numeric strings), False for NaN/None/""
    if x is None:
        return False
    try:
        v = float(x)
...
# Use the latest Basis_Date in the data for the title (NOT today's date)
basis_dt = pd.to_datetime(corn.get("Basis_Date"), errors="coerce").max()
...
if can_interpolate:
    ...
    # Map centered on data (with explicit HTTPS tiles)
    m = folium.Map(
        location=[float(np.mean(Y)), float(np.mean(X))],
        zoom_start=6,
        tiles=None,            # IMPORTANT: we will add HTTPS tiles explicitly
        control_scale=True,
    )
...
<div class="map-card map-title">
  <h3 align="center" style="margin:0;"><b>Ethanol Corn <br> Nearby Basis Map {selected}</b></h3>
</div>
"""
...
archive_path = Path(rf"C:/Users/ehakm/OneDrive/Documents/Sankey/Corn Map/ethanol_map_{today}.html")
...
# Write ONLY to docs/ for GitHub Pages
PUBLISH_ROOTS = [
    REPO_DIR / "docs"
]
...
def write_pages_bundle(root_dir: Path, html: str) -> None:
    ...
    # 1) ALWAYS overwrite the "latest" file (no re-render)
    out_html = static_dir / "Current_Basis.html"
    out_html.write_text(html, encoding="utf-8")
    print(f"✅ Map saved -> {out_html}")

    # 2) ALSO save a daily snapshot (no re-render)
    snap_day = datetime.now().strftime("%Y%m%d")  # e.g., 20251229
    snap_html = static_dir / f"Current_Basis_{snap_day}.html"
    snap_html.write_text(html, encoding="utf-8")
    print(f"✅ Snapshot saved -> {snap_html}")


    # 3) Redirect inside static_data (so /static_data/ always lands on latest)
    (static_dir / "index.html").write_text(
        """<!doctype html><html><head><meta charset="utf-8">
<script>
  window.location.replace("Current_Basis.html?ts=" + Date.now());
</script>
</head><body></body></html>""",
        encoding="utf-8"
    )
    print(f"✅ Redirect index -> {static_dir / 'index.html'}")

    # 4) Redirect at REPO ROOT (so /ELHApp-backend/ always lands on latest)
    (root_dir / "index.html").write_text(
        """<!doctype html><html><head><meta charset="utf-8">
<script>
  window.location.replace("./static_data/Current_Basis.html?ts=" + Date.now());
</script>
</head><body></body></html>""",
        encoding="utf-8"
    )
    print(f"✅ Root redirect -> {root_dir / 'index.html'}")

    # 5) Copy images into published folder (optional)
    for src in [LOCAL_MARKET_IMG, LOCAL_PERSONAL_IMG]:
        p = Path(src)
        if p.exists():
            shutil.copy(p, static_dir / p.name)
            print(f"✅ Image copied -> {static_dir / p.name}")
        else:
...
# ==========================
# SANITY CHECK: Adj_Basis > 2.50
# ==========================
THRESH = 2.50

# Make sure we're only looking at numeric Adj_Basis rows
tmp = numeric_rows.copy()
tmp["Adj_Basis"] = pd.to_numeric(tmp["Adj_Basis"], errors="coerce")

hits = (
    tmp.loc[tmp["Adj_Basis"] > THRESH, ["EPM", "Ownership", NAME_COL, "City", "State", "Adj_Basis", "Basis_Date"]]
      .sort_values("Adj_Basis", ascending=False)
)

print("\n" + "#" * 60)
print(f"SANITY CHECK: Adj_Basis > {THRESH:.2f}  (count={len(hits)})")

if hits.empty:
    print("None.")
else:
    # Pretty print each location name (and details so it’s actually useful)
    for r in hits.to_dict("records"):
        print(f"{r.get(NAME_COL,'')}"
              f" | {r.get('City','')}, {r.get('State','')}"
              f" | Adj_Basis={float(r.get('Adj_Basis')):,.2f}"
              f" | EPM={r.get('EPM','')}"
              f" | {r.get('Ownership','')}"
              f" | {r.get('Basis_Date','')}")
