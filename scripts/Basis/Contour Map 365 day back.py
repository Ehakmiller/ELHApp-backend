...
exec(open(r"C:\Users\ehakm\OneDrive\Documents\Python Code\Ethanol db\Basis_dataframe\Basis_df365day.py",
...
<div class="map-card map-title">
  <h3 align="center" style="margin:0;"><b>Ethanol Corn <br> Nearby Basis Map (365 Days Ago) {selected}</b></h3>
</div>
"""
...
archive_path = Path(rf"C:/Users/ehakm/OneDrive/Documents/Sankey/Corn Map/ethanol_map_365DaysAgo_{today}.html")
...
PUBLISH_ROOTS = [REPO_DIR]   # publish ONLY to repo root
...
def write_pages_bundle(root_dir: Path, html: str) -> None:
    ...
    snap_dir = static_dir / "365_days_ago"
    snap_dir.mkdir(parents=True, exist_ok=True)

    # Save the file the redirect will point to
    out_html = snap_dir / "Current_Basis_365DaysAgo.html"
    out_html.write_text(html, encoding="utf-8")
    print(f"✅ 365-days-ago map saved -> {out_html}")

    # Redirect only controls /static_data/365_days_ago/
    (snap_dir / "index.html").write_text(
        """<!doctype html><html><head><meta charset="utf-8">
    <script>
      window.location.replace("Current_Basis_365DaysAgo.html?ts=" + Date.now());
    </script>
    </head><body></body></html>""",
            encoding="utf-8"
        )
    print(f"✅ 365-days-ago redirect index -> {snap_dir / 'index.html'}")
