...
TABLE_CONFIGS = (
    {
        ...
        "header_markers": ("CI Applicant", "Approved CI (gCO2e/MJ)"),
        "epm_aliases": ("EPM", "EPM#", "epm", "EPM_NUMBER"),
        "plant_candidates": ("Facility Name", "CI Applicant", "Plant", "Name"),
        "ci_patterns": (r"approved\s+ci", r"canadian.*ci", r"\bci\b"),
        "epm_overrides": {},
    },
    {
        ...
        "epm_aliases": ("EPM", "EPA"),
        "plant_candidates": ("company", "Plant", "Facility", "Facility Name", "Name"),
...
def find_header_row(sheet_name: str, markers: tuple[str, ...]) -> int:
    preview = pd.read_excel(WORKBOOK, sheet_name=sheet_name, header=None, nrows=30, dtype=object)
...
def load_sheet(config: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "preview": df[preview_cols].head(8) if preview_cols else pd.DataFrame(),
...
def main() -> None:
    ...
    results = [load_sheet(config) for config in TABLE_CONFIGS]
